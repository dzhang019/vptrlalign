...
def train_rl(
    ...
    for iteration in range(num_iterations):
        ...
        while step_count < rollout_steps:
            ...
            for env_i in range(num_envs):
                if not done_list[env_i]:
                    ...
                    # (A) Call get_action_and_training_info with taken_action=None => we sample
                    #     This also returns new_hidden_state
        ...
        # ==== 5) Training unroll: re-play each env's partial trajectory to get (log_prob, vpred, etc.) ====
        ...
        if len(transitions_all) == 0:
            print("[Iteration {iteration}] No transitions collected, skipping update.")
            continue
        ...
        for t in transitions_all:
            ...
            # KL regularization
            # We do: kl_loss = compute_kl_loss(cur_pd, old_pd)
...
def train_unroll(agent, pretrained_policy, rollout, gamma=0.999, lam=0.95):
    ...
    for t in range(T):
        ...
        # (A) Force the same action => get log_prob, v_pred, distribution
        minerl_action, pi_dist, v_pred, log_prob, hid_out = agent.get_action_and_training_info(
            minerl_obs=obs_t,
            hidden_state=hid_t,
            stochastic=False,       # ignoring stoch since we have forced action
            taken_action=act_t      # the action we actually took
        )
        # 'minerl_action' is basically the same as 'act_t', just re-run. 
        # We want 'pi_dist' as 'cur_pd' for KL if needed.

        # (B) pretrained distribution => typically you do a separate forward pass 
        #     or a method to get old distribution from pretrained. 
        # For example:
        with th.no_grad():
        ...
        # old_pd is the old distribution => we can do compute_kl_loss(pi_dist, old_pd)
    ...
    # (C) local GAE
    # If not done, we bootstrap from final next_obs
    if not transitions[-1]["done"]:
        # we can do a final forward pass for next_obs
        # or rely on old code. Let's do a quick approach:
        with th.no_grad():
            _, _, v_next, _, _ = agent.get_action_and_training_info(
                minerl_obs=transitions[-1]["next_obs"],
                hidden_state=rollout["hidden_states"][-1],  # or hid_out
                stochastic=False,
                taken_action=None  # we just want the value, so no forced action
            )
