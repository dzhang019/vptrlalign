...
class HumanSurvival(HumanControlEnvSpec):
    ...
    #     ]
    def create_server_initial_conditions(self) -> List[Handler]:
    # Implement a balanced approach - higher chance of night starts to compensate for bias
        if random.random() < 0.7:  # 70% chance of night start
            # Night time (14000-22000)
            start_time = random.randint(13000, 23000)
        else:
            # Day time (0-12000)
            start_time = random.randint(0, 12000)
        
        return [
            handlers.TimeInitialCondition(
                allow_passage_of_time=True,
                start_time=start_time  # Use the randomized time
            ),
            handlers.SpawningInitialCondition(allow_spawning=True),
        ]
